...
self.addEventListener('install', (e) => {
  ...
  e.waitUntil(
    ...
    }).catch(function(err) {console.log('ServiceWorker caching app shell failed: ', err);}));
});
...
self.addEventListener('activate', (e) => {
  ...
  e.waitUntil(
    ...
    }).catch(function(err) {console.log('ServiceWorker not active: ', err);}));
});
  ...
  caches.match(e.request.clone()).then((response) => {
    return response || fetch(e.request.clone()).then((r2) => {
      return caches.open(dataCacheName).then((cache) => {
        ...
        return r2.clone();
      }).catch(function(err) {console.log('ServiceWorker fetch URL failed: ', err);})
  });
  })
  );
});